...
The Trade entity represents a fully specified trade along a route. This entity supplies all the 
information necessary to craft a router transaction.

# Example

```tsx twoslash
import { ChainId, Token, CurrencyAmount, TradeType } from '@razorlabs/swap-sdk-core'
import { Pair, Trade, Route } from '@razorlabs/amm-sdk'
...
The [path](/docs/sdks/amm/reference/route#path) property of the route should be passed as the path parameter to router functions.
...
`TradeType.EXACT_INPUT` corresponds to `swap_exact_for` router functions. `TradeType.EXACT_OUTPUT` corresponds to `swap_for_exact` router functions.
...
For exact input trades, this value should be passed as `amount_in` to router functions. For exact 
output trades, this value should be multiplied by a factor &gt;1, representing slippage tolerance, 
and passed as `amount_in_max` to router functions.
...
For exact output trades, this value should be passed as `amount_out` to router functions. For 
exact input trades, this value should be multiplied by a factor &lt;1, representing slippage 
tolerance, and passed as `amount_out_min` to router functions.
...
Returns the minimum amount of the output token that should be received from a trade, given the 
slippage tolerance.
...
Returns the maximum amount of the input token that should be spent on the trade, given the 
slippage tolerance.
...
Given a list of pairs, and a fixed amount in, returns the top `maxNumResults` trades that go 
from an input token amount to an output token, making at most `maxHops` hops.
